...
app.post("/api/telegram/test", async (req, res) => {
  ...
  console.log(`[Telegram Test] request -> chatId=${cleanChatId} token=${maskToken(cleanToken)}`);
  ...
  try {
    ...
    console.error(`[Telegram Test] Error while sending test message: ${err && err.message}`, err);
    res.status(500).json({
...
app.post("/api/telegram/send", async (req, res) => {
  ...
  console.log(`[Telegram Send] request -> chatId=${cleanChatId} token=${maskToken(cleanToken)} replyTo=${replyToMessageId ? replyToMessageId : 'none'}`);
  ...
  try {
    ...
    if (!data.ok && (data.description || "").toLowerCase().includes("parse")) {
      ...
      console.debug("[Telegram Send] fallback (plain-text) Telegram API response:", data);
    }
    ...
    console.error(`[Telegram Send] Exception while sending: ${err && err.message}`, err);
    res.status(500).json({
