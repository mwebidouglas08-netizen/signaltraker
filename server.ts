...
function sanitizeTelegramCredentials(botToken: string, chatId: string) {
  ...
  // 5. If it is a alphanumeric handle and does not start with @ or -100 (for private channels), prepend @
  if (cleanChatId && !cleanChatId.startsWith("@") && !cleanChatId.startsWith("-") && isNaN(Number(cleanChatId))) {
    cleanChatId = "@" + cleanChatId;
  }

  // 6. Support positive numeric IDs that represent channels (e.g., "8071150468" -> "-1008071150468")
  // Private channel IDs are returned as positive integers by some bots/tools, but when sending via Telegram Bot API, they require "-100" prepended.
  if (/^\d{8,12}$/.test(cleanChatId)) {
